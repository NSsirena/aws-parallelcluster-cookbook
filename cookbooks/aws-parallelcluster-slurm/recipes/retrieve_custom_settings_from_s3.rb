...
if kitchen_test?
  local_path = node['cluster']['config'].dig(:Scheduling, :SchedulerSettings, :LocalPath)
else
  local_path = "#{node['cluster']['slurm']['install_dir']}/etc/pcluster/custom_slurm_settings_include_slurm.conf"
end

s3_object 'Retrieve Custom Slurm Settings' do
